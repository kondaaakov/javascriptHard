...
class Cart {
    ...
    addToCart(dataOfElem) {
        ...
        if (findItem) {
            ...
            this._makeNewListenerOnItem(dataOfElem);
        }
        this.renderTotalPrice();
    }
    ...
    _makeNewListenerOnItem(elem) {
        let removeIcon = document.querySelector(`.cart_product_remove[data-id="${elem.id}"]`);
        removeIcon.addEventListener('click', event => this.#removeFromCart(+event.target.dataset.id));
    }

    clickListeners() {
        const removeEls = document.querySelectorAll('.cart_product_remove');
        removeEls.forEach(elem => elem.addEventListener('click',
            event => this.#removeFromCart(+event.target.dataset.id)));

        const addBtnEls = document.querySelectorAll('.btn_add_to_cart');
        addBtnEls.forEach(elem => elem.addEventListener('click',
            event => this.addToCart(event.target.dataset)));
    };
}
